/**
 * manual-test.js — Quick smoke test for the audit pipeline
 *
 * Runs audits on example.com (simple) and wikipedia.org (complex),
 * then saves a JSON report to ./test-report.json.
 *
 * Usage: npm test   (or: node test/manual-test.js)
 */
...
async function runTests() {
  ...
  if (!process.env.OPENAI_API_KEY) {
    console.log('⚠️  No OpenAI API key found. Running without LLM analysis.\n');
  }
  ...
  try {
    ...
    // Test 2: More complex site (usually has more violations)
    console.log('\n' + '='.repeat(60));
