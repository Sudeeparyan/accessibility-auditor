...
// ─── Lambda-compatible Puppeteer loading ─────────────────────────────────────
// In AWS Lambda, we use @sparticuz/chromium (bundled as dependency) instead
// of the stock puppeteer package (which bundles a 280MB Chromium binary).
...
let axeSource;
try {
  // Try require.resolve first (works locally and when axe-core is a webpack external)
  const axePath = require.resolve('axe-core');
  axeSource = fs.readFileSync(axePath, 'utf8');
  console.log(`axe-core loaded (${(axeSource.length / 1024).toFixed(0)}KB)`);
} catch (err) {
  // Fallback: search common Lambda node_modules paths
  const fallbackPaths = [
    path.join(__dirname, '..', 'node_modules', 'axe-core', 'axe.min.js'),
    path.join(__dirname, 'node_modules', 'axe-core', 'axe.min.js'),
    '/var/task/node_modules/axe-core/axe.min.js',
    path.join(process.cwd(), 'node_modules', 'axe-core', 'axe.min.js'),
  ];
  for (const fp of fallbackPaths) {
    try {
      axeSource = fs.readFileSync(fp, 'utf8');
      console.log(`axe-core loaded via fallback: ${fp} (${(axeSource.length / 1024).toFixed(0)}KB)`);
      break;
    } catch (e) { /* try next */ }
  }
  if (!axeSource) {
    console.error('Failed to load axe-core from any path:', err.message);
  }
}
...
class AccessibilityScraper {
  ...
  async initialize() {
    ...
    if (IS_LAMBDA) {
      // Lambda: use @sparticuz/chromium args + executable
      launchArgs.push(...chromium.args);
      const executablePath = await chromium.executablePath();
      console.log(`Lambda mode: using Chromium at ${executablePath}`);
      ...
      // Local: use stock puppeteer with bundled Chromium
